...
const RULES: Partial<Record<LeafAssertion['type'], GuidanceRule>> = {
  ...
  'dhcp-lease': {
    question:
      'La demande d adresse atteint-elle un serveur ? Jusqu ou la diffusion se propage-t-elle ?',
    checks: [
  ...
  'has-route': {
    ...
    checks: [
      ...
      'verifier la presence et la validite de la passerelle par defaut',
    ],
  ...
  'service-status': {
    ...
    checks: [
      ...
      'verifier ses dependances',
      'controler son mode de demarrage',
    ],
  ...
  'file-permissions': {
    ...
    checks: [
      'afficher le proprietaire et les droits',
      'verifier l appartenance aux groupes concernes',
    ],
  ...
  unchanged: {
    ...
    checks: [
      'revenir sur les changements hors perimetre',
      'privilegier la correction la plus ciblee',
    ],
...
const DEFAULT_RULE: GuidanceRule = {
  ...
  checks: [
    'reprendre la verification depuis les couches basses',
    'comparer avec un element equivalent qui fonctionne',
  ],
